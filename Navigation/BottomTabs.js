...
const BottomTabs = () => {
  ...
  return (
    <Tab.Navigator
      screenOptions={({ route }) => ({
        tabBarIcon: ({ focused, color, size }) => {
          ...
          if(route.name === 'Search') return <Feathericons name={iconName} size={30} color={color}/>
          if(route.name === 'GenreList') return <Entypoicons name={iconName} size={30} color={color}/>
    ...
    >
      ...
      <Tab.Screen name="GenreList" component={GenreList} options={{ headerShown: false }}/>
    </Tab.Navigator>
  );
}
